 ...
 * Nota:
 * para el ambiente oficial se cambia el:http://192.168.2.21/ por http://dte.integrasystem.cl/
*/
...
class dte {
	...
	function post_emitir_dte($objEnJson) { //DTE ENITIDO
		...
		curl_setopt($curl, CURLOPT_URL, 'http://192.168.2.21/api/dte/documentos/emitir');
	...
	function post_genera_dte($ve_objEnJson){	//FUNCION GENERA_DTE
		...
		curl_setopt($curl_genera, CURLOPT_URL, 'http://192.168.2.21/api/dte/documentos/generar?getXML=1');
	...
	function post_genera_pdf($ve_dte,$ve_folio,$ve_emisor){
		...
		curl_setopt($curl_pdf, CURLOPT_URL, 'http://192.168.2.21/api/dte/dte_emitidos/pdf/'.$ve_dte.'/'.$ve_folio.'/'.$ve_emisor.'?usarWebservice=true');
	...
	function actualizar_estado($ve_dte,$ve_folio,$ve_emisor){
	    ...
	    curl_setopt($ch,CURLOPT_URL,'http://192.168.2.21/api/dte/dte_emitidos/actualizar_estado/'.$ve_dte.'/'.$ve_folio.'/'.$ve_emisor.'?usarWebservice=true');
	...
	function enviar_sii($ve_folio,$ve_tipo_doc,$ve_emisor){
		...
		$header = Array("Content-Type: application/json","User-Agent: SowerPHP Network_Http_Rest","Authorization: Basic ".base64_encode($hash.':X'));
		curl_setopt($ch, CURLOPT_HTTPHEADER, $header);
	    curl_setopt($ch,CURLOPT_URL,'http://192.168.2.21/api/dte/dte_emitidos/enviar_sii/'.$ve_tipo_doc.'/'.$ve_folio.'/'.$ve_emisor.'');
	    curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
	...
	function eliminar_dte($ve_folio,$ve_tipo_doc,$ve_emisor){
		...
		$header = Array("Content-Type: application/json","User-Agent: SowerPHP Network_Http_Rest","Authorization: Basic ".base64_encode($hash.':X'));
		curl_setopt($ch, CURLOPT_HTTPHEADER, $header);
	    curl_setopt($ch,CURLOPT_URL,'http://192.168.2.21/api/dte/dte_emitidos/eliminar/'.$ve_tipo_doc.'/'.$ve_folio.'/'.$ve_emisor.'');
	    curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
	...
	function acuse_comercial($ve_folio,$ve_tipo_doc,$ve_emisor){
	    ...
	    if($this->hash == '')
	        $hash = 'nLAwgOlKsPH5I3VCAO8BibDnl2pr5Q5j';
	            ...
	            $header = Array("Content-Type: application/json","User-Agent: SowerPHP Network_Http_Rest","Authorization: Basic ".base64_encode($hash.':X'));
	            curl_setopt($ch, CURLOPT_HTTPHEADER, $header);
	            curl_setopt($ch,CURLOPT_URL,'http://192.168.2.21/api/dte/dte_emitidos/info/'.$ve_tipo_doc.'/'.$ve_folio.'/'.$ve_emisor.'');
	            curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
	...
	function listar_dte($objEnJson,$rutemisor) { //DTE ENITIDO
	    ...
	    if($this->hash == '')
	        $hash = 'nLAwgOlKsPH5I3VCAO8BibDnl2pr5Q5j';
	            ...
	            $header = Array("Content-Type: application/json","User-Agent: SowerPHP Network_Http_Rest","Authorization: Basic ".base64_encode($hash.':X'));
	            curl_setopt($curl, CURLOPT_HTTPHEADER, $header);
	            curl_setopt($curl, CURLOPT_URL, 'http://192.168.2.21/api/dte/dte_emitidos/buscar/'.$rutemisor);
	            curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
	            curl_setopt($curl, CURLOPT_HEADER, 1);
